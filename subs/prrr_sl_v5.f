...
C
C
	SUBROUTINE PRRR_SL_V5(PR,RR,BFCR,FF,WSE,WCR,
	1                     HN,HNST,NLEV,ZHYD,
	1                     DI,DIST,N_DI,
	...
	IMPLICIT NONE
C
C Altered 03-Mar-2004 : NU installed in CALL. Computation of FF cooling revised.
...
C
	...
	END DO
C
C Compute Free-Free cooling.
C
C Compute free-free gaunt factors. Replaces call to GFF in following DO loop.
C
	CALL GFF_VEC(GFF_VAL,NU_CONT,T,ZHYD,ND)
C
C The opacity is evaluated at NU_CONT. However, the stimulated emission term
C should be evaluated at NU, since we correct CHI and ETA for the change
C in freqency but assuming a constant cross-section.
C
C The constant in T2 is 4PI x 1.0E-10.
C

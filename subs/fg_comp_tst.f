...
C we insert additional points so that we solve the transfer equation using
C the same techniques.
C
	R_RAY(1:ND)=R(1:ND)
	V_RAY(1:ND)=V(1:ND)
	SIGMA_RAY(1:ND)=SIGMA(1:ND)
	ETA_RAY(1:ND)=ETA(1:ND)
	CHI_RAY(1:ND)=CHI(1:ND)
	ETAL_RAY(1:ND)=ETAL(1:ND)
	CHIL_RAY(1:ND)=CHIL(1:ND)
C
C Enter loop to perform integration along each ray.
C
	DO LS=1,MIN(ND+NC-1,NP)
	  NI=ND-(LS-NC-1)
	  IF(LS .LE. NC)NI=ND
	  IF(NI .EQ. ND)THEN
	    NIEXT=ND
	  ...
	  END IF
C
C Final ray. Ensure we have ar least 3 pints for tridiagonal solution.
C
	  IF(NI .EQ. 2)THEN
	     R_RAY(4:10)=R_RAY(2:8)
	     V_RAY(4:10)=V_RAY(2:8)
	     SIGMA_RAY(4:10)=SIGMA_RAY(2:8)
	     ETA_RAY(4:10)=ETA_RAY(2:8)
	     CHI_RAY(4:10)=CHI_RAY(2:8)
...
C
	    ...
	    END IF
	    DO I=1,NIEXT
	      TCHI(I)=CHI_RAY(I)+CHIL_RAY(I)*T1
	      SOURCE(I)=(ETA_RAY(I)+ETAL_RAY(I)*T1)/TCHI(I)
	    END DO
...
C
	    ...
	    ELSE
	      CALL DERIVCHI(dCHIdR,TCHI,R_RAY,NIEXT,METHOD)
	      CALL NORDTAU(DTAU,TCHI,Z,R_RAY,dCHIdR,ND)
	    END IF
...
C
	    ...
	    END DO
C
	    IF(LS .EQ. ND+NC-1)THEN
	        JNU(1,ML)=JNU(1,ML)+JQW(1,LS)*AV(1)
	        KNU(1,ML)=KNU(1,ML)+KQW(1,LS)*AV(1)
	        JNU(2,ML)=JNU(2,ML)+JQW(2,LS)*AV(4)
	        KNU(2,ML)=KNU(2,ML)+KQW(2,LS)*AV(4)
	        HNU(1,ML)=HNU(1,ML)+HQW(1,LS)*CV(2)
	        NNU(1,ML)=NNU(1,ML)+NQW(1,LS)*CV(2)
	    ELSE
	      DO I=1,NI
...
C
	    ...
	    ELSE
	      CALL DERIVCHI(dCHIdR,CHI,R_RAY,NIEXT,METHOD)
	      CALL NORDTAU(DTAU,CHI,Z,R_RAY,dCHIdR,NI)
	    END IF
C
...
C
	    IF(LS .EQ. ND+NC-1)THEN
	      JNU(1,NLF+1)=JNU(1,NLF+1)+JQW(1,LS)*XM(1)
	      KNU(1,NLF+1)=KNU(1,NLF+1)+KQW(1,LS)*XM(1)
	      JNU(2,NLF+1)=JNU(2,NLF+1)+JQW(2,LS)*XM(4)
	      KNU(2,NLF+1)=KNU(2,NLF+1)+KQW(2,LS)*XM(4)
	      HNU(1,NLF+1)=HNU(1,NLF+1)+HQW(1,LS)*CV(2)
	      NNU(1,NLF+1)=NNU(1,NLF+1)+NQW(1,LS)*CV(2)
	    ELSE
	      DO I=1,NI
...
C
	  ...
	  END DO
	  JNU(1,NLF+1)=JNU(1,NLF+1)+JQW(1,LS)*IBOUND*DNU
	END IF
C

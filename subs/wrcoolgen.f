...
C
	SUBROUTINE WRCOOLGEN(BFCRC2,FFC2,COLC2,DIECOOL,X_COOL,
...
C
	...
	REAL*8 BFCRC2(NC2,ND),FFC2(ND),COLC2(ND),DIECOOL(ND),X_COOL(ND)
...
C
	DO J=MS,MF
	  ...
	  END DO
	  NETRR(J)=NETRR(J)+FFC2(J)+COLC2(J)+DIECOOL(J)+X_COOL(J)
	  TOTRR(J)=TOTRR(J)+ABS(FFC2(J))+ABS(COLC2(J))+
	1            ABS(DIECOOL(J))+ABS(X_COOL(J))
	END DO
C
	RETURN

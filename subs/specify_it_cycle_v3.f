...
! consistent.
!
	SUBROUTINE SPECIFY_IT_CYCLE_V3(MAIN_COUNTER,COMPUTE_BA,LAMBDA_ITERATION,FIXED_T,
	1                               NEXT_NG,NEXT_AV)
	USE UPDATE_KEYWORD_INTERFACE
	IMPLICIT NONE
!
! Altered 19-Nov-2016: Changed to V3. Added NEXT_NG and NEXT_AV to call.
...
!
	IF(IT_OPTION(1:4) .EQ. 'NORM' .OR.  IT_OPTION(1:4) .EQ. 'NONE')THEN
	  CLOSE(LU_IN)
	  RETURN
	ELSE IF(IT_OPTION(1:5) .EQ. 'DO_NG')THEN
	  NEXT_NG=MAIN_COUNTER+1 
	ELSE IF(IT_OPTION(1:5) .EQ. 'DO_AV')THEN
	  NEXT_AV=MAIN_COUNTER+1 

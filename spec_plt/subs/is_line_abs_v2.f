...
C on 4-30-92 by SRM.
C
	SUBROUTINE IS_LINE_ABS_V2(WAVE,FLUX,NLAM,V_TURB,LOG_NTOT,T_IN_K,IS_FILE)
	IMPLICIT NONE
C
C Altered 12-Feb-2015 : Inserted IS_FILE in call; Changed to V2
...
C
	OPEN(UNIT=10,FILE=TRIM(IS_FILE),ACTION='READ',STATUS='OLD',IOSTAT=IOS)
        IF(IOS .NE. 0)THEN
	   WRITE(6,*)'Unable to open IS_LINE_LIST FILE: ',TRIM(IS_FILE)
	   RETURN
	END IF
	I=0; NLINES=0
	DO WHILE(1 .EQ. 1)

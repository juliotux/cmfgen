...
!
	SUBROUTINE OBS_FRAME_SUB_V8(ETA_CMF,CHI_CMF,FREQ_CMF,
	1            R,V,T,ED,ND,NCF,
...
!
	IF(WRITE_RTAU)THEN
	   ...
	   ACCESS_F=5
	   I=WORD_SIZE*(NP+1)/UNIT_SIZE; J=82
	   CALL WRITE_DIRECT_INFO_V3(NP,I,'20-Aug-2000','RTAU_DATA',J)
	   OPEN(UNIT=82,FILE='RTAU_DATA',FORM='UNFORMATTED',
     > ACCESS='DIRECT',STATUS='NEW',RECL=I,IOSTAT=IOS)
	   WRITE(82,REC=3)ACCESS_F,NOS,NP
	   WRITE(82,REC=ACCESS_F)(P(I),I=1,NP)
	   DO ML=1,NOS
	      WRITE(82,REC=ACCESS_F+ML)(RTAU(I,ML),I=1,NP),OBS_FREQ(ML)
	   END DO
	   CLOSE(UNIT=82)
!
!       write ZTAU
!
	   ...
	   ACCESS_F=5
	   I=WORD_SIZE*(NP+1)/UNIT_SIZE; J=82
	   CALL WRITE_DIRECT_INFO_V3(NP,I,'20-Aug-2000','ZTAU_DATA',J)
	   OPEN(UNIT=82,FILE='ZTAU_DATA',FORM='UNFORMATTED',
     > ACCESS='DIRECT',STATUS='NEW',RECL=I,IOSTAT=IOS)
	   WRITE(82,REC=3)ACCESS_F,NOS,NP
	   WRITE(82,REC=ACCESS_F)(P(I),I=1,NP)
	   DO ML=1,NOS
	      WRITE(82,REC=ACCESS_F+ML)(ZTAU(I,ML),I=1,NP),OBS_FREQ(ML)
	   END DO
	   CLOSE(UNIT=82)
	END IF
!

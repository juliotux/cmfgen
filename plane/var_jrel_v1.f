...
! JNU and r^2 HNU.
!
	SUBROUTINE VAR_JREL_V1(ETA,CHI,ESEC,THETA,V,SIGMA,R,
...
! 
!
	IF(INIT)CALL ALLOC_MOD_VAR_JREL(ND)
!
...
!
	...
	1            CON_dKdNUH(I)*KMID_ON_J(I))/(P_H(I)+W(I))
	    ...
	    EPS_PREV_A(I)=DELTAH(I)*(CON_dNdNUH(I)*RSQN_ON_RSQJ_PREV(I)+
...
!
	DO I=1,ND-1
	  HU(I)=GAM_RSQ(I+1)*(FEDD(I+1)+VdHdR_TERM(I+1))*Q(I+1)/(P_H(I)+W(I))/DTAU_H(I)
	  HL(I)=GAM_RSQ(I)*(FEDD(I)+VdHdR_TERM(I))*Q(I)/(P_H(I)+W(I))/DTAU_H(I)
...
!
	CALL TUNE(1,'VAR_JREL_V1')
	CALL EDD_JREL_VAR_V1(KI,RHS_dHdCHI,
	1          R,SIGMA,CHI,ESEC,FEDD,
	1          DBB,DIF,J_AT_INB_EQ_B,METHOD,ND,NM_KI)
	CALL TUNE(2,'VAR_JREL_V1')
!
...
!
	...
	GAM_RSQHNU_PREV(:)=GAM_RSQHNU(:)
        ...
        H_ON_J_PREV(:)=H_ON_J(:)
        N_ON_J_PREV(:)=N_ON_J(:)
        RSQN_ON_RSQJ_PREV(:)=RSQN_ON_RSQJ(:)
        KMID_ON_J_PREV(:)=KMID_ON_J(:)
!
	HBC_PREV=HBC
	NBC_PREV=NBC
	IN_HBC_PREV=IN_HBC

	...
	IF(INIT .AND. .NOT. FIRST_TIME)THEN
	  DO I=1,ND
	    IF(R(I) .NE. R_EXT(ND_ADD+I))THEN
	      NEW_R_GRID=.TRUE.
	      J=ERROR_LU()
	      WRITE(J,*)'Warning: Updating RGRID in FG_J_CMF_V13'
	      EXIT
	    END IF
	  END DO
...
!
	  ...
	  END DO
	  J=ERROR_LU()
	  WRITE(J,'(A,I6)')' Maximum number of points along ray in FG_J_CMF_V13 is',NRAY_MAX
!
...
!
	    ...
	    IF(IOS .NE. 0)THEN
	      WRITE(LUER,*)'Error allocating DIFFERENCE block in FG_J_CMF_V13: Status=',IOS
	      STOP	
            END IF
	  END IF
!
...
!
	...
	1            Q,QH,DTAU(1,LS),SOURCE_RAY,DIF_OR_ZF,DBC,IC,LS,NC,NI)
	      XM(1)=-IBOUND(LS)
!

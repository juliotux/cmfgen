...
!
	SUBROUTINE JGREY_HUB_DDT_V2(RJ,RSQ_HFLUX,CHI,R,VEL,SIGMA,POPS,
...
!
	DO I=1,ND
	  TA(I)=RJ(I)+(E_RAD_DECAY(I)-WORK(I))/CHI(I)
	END DO
!
	...
	DO I=1,ND
	  IF(TA(I) .LE. 0.0D0 .OR. RJ(I) .LE. 0.0D0)THEN
	    ...
	    ELSE
	      ...
	      WRITE(6,'(4X,A,I4,20X,A,ES12.4,10X,A,ES12.4)')'I=',I,'B=',TA(I),'J=',RJ(I)
              WRITE(6,'(X,A,ES12.4,3X,A,ES12.4,6X,A,ES12.4)')
	1               'WORK=',WORK(I),'T_FROM_J=',T_FROM_J(I),'OLD_T=',OLD_T(I)
	      DO J=1,ND
	        WRITE(145,'(I3,5ES14.4)')J,T_FROM_J(J),OLD_T(J),WORK(J),RJ(J),TA(J)
	      END DO
	      STOP
	    END IF
...
!
	DO I=1,ND
	  RJ(I)=RJ(I)+(E_RAD_DECAY(I)-WORK(I))/CHI(I)
	END DO
!
	RETURN
	END

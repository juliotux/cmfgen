...
!
	SUBROUTINE MOM_JREL_V6(ETA_SM,CHI_SM,ESEC_SM,V_SM,SIGMA_SM,R_SM,
...
!	    IF(I .EQ. ND_SM-1)IT1=20
	  ...
	  ND=K
	  IF(ND .NE. ND_SM)THEN
	    WRITE(LUER,*)'Warning in MOM_JREL_V6: we have adjusted ND for more accuracy'
...
!	    IF(I .EQ. ND_SM-1)IT1=20
	  ...
	  END DO
	  IF(ND .NE. K)THEN
	    WRITE(LUER,*)'Error in MOM_JREL_V6'
...
!
	  IF(OUTER_BND_METH .EQ. 'HONJ')THEN
	    ...
	    VC(1)=0.0D0
	  ELSE
	    WRITE(LUER,*)'Error in MOM_JREL_V6: Only outer boundary method implemented is HONJ'
...
!
	...
	1                    +T1*(RSQ_HP-IN_HBC_PREV)
	      XM(ND-1)=XM(ND-1)-RSQ_JP*TC(ND-1)
!
! Test to see if improves stability.
!
...
!
	    ...
	    IF(COUNT .EQ.  100)THEN
	      WRITE(LUER,*)'Error in MOM_J_REL_V6: excessive iteration count.'

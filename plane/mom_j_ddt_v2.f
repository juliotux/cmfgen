!
! Data module for MOM_J_DDT_V2. Data placed in this module is automatically
! saved between subroutine calls..
!
	MODULE MOD_MOM_J_DDT_V2
	IMPLICIT NONE
!
! Dimensioned ND_SM,4
...
!
	SUBROUTINE MOM_J_DDT_V2(ETA_SM,CHI_SM,ESEC_SM,
	1            V_SM,R_SM,F_SM,
...
!
	IF(INIT)THEN
	  IF(DO_TIME_VAR)THEN
	    ...
	    IF(FIRST_TIME)THEN
	       WRITE(LUER,'(A,ES12.4)')' RECIP_CDELTAT=',RECIP_CDELTAT
	       WRITE(LUER,'(A,ES12.4)')'     ROLD_ON_R=',ROLD_ON_R
	    END IF
	  ELSE
	    RECIP_CDELTAT=0.0D0
	    ROLD_ON_R=0.0D0

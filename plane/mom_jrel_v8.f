...
!	    IF(I .EQ. ND_SM-1)IT1=20
	  ...
	  ND=K
	  IF(ND .NE. ND_SM)THEN
	    WRITE(LUER,*)'Warning in MOM_JREL_V8: we have adjusted ND for more accuracy'
...
!
	  IF(N_TYPE .NE. 'G_ONLY' .AND. N_TYPE .NE. 'N_ON_J' .AND. ND .NE. ND_SM)THEN
	    LUER=ERROR_LU()
	    WRITE(LUER,*)'Error in MOM_JREL_V8'
...
!
	  IF(OUTER_BND_METH .EQ. 'HONJ')THEN
	    ...
	    VC(1)=0.0D0
	  ELSE
	    WRITE(LUER,*)'Error in MOM_JREL_V8: Only outer boundary method implemented is HONJ'
...
!
	    ...
	    IF(COUNT .EQ.  100)THEN
	      WRITE(LUER,*)'Error in MOM_J_REL_V8: excessive iteration count.'

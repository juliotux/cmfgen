...
!
	  ...
	  IF(IOS .NE. 0)ALLOCATE (CHI(ND,STAT=IOS)
	  IF(IOS .NE. 0)ALLOCATE (ESEC(ND,STAT=IOS)
	  IF(IOS .NE. 0)ALLOCATE (V(ND,STAT=IOS)			!in km/s
	  IF(IOS .NE. 0)ALLOCATE (SIGMA(ND,STAT=IOS)		!dlnV/dlnR
	  IF(IOS .NE. 0)ALLOCATE (R(ND,STAT=IOS)			!in units of 10^10 cm
!
	  IF(IOS .NE. 0)ALLOCATE (FEDD(ND,STAT=IOS)			!J/K at nodes
	  IF(IOS .NE. 0)ALLOCATE (GEDD(ND,STAT=IOS)			!N/G at midpoints
	  IF(IOS .NE. 0)ALLOCATE (H_ON_J(ND,STAT=IOS)			!H/J at nodes
	  IF(IOS .NE. 0)ALLOCATE (N_ON_J(ND,STAT=IOS)			!N/J at nodes
	  IF(IOS .NE. 0)ALLOCATE (RSQN_ON_RSQJ(ND,STAT=IOS)		!N/J at nodes
	  IF(IOS .NE. 0)ALLOCATE (KMID_ON_J(ND,STAT=IOS)		!
!
	  IF(IOS .NE. 0)ALLOCATE (R_PNT(ND,STAT=IOS)			!
	  IF(IOS .NE. 0)ALLOCATE (J_INDX(ND,STAT=IOS)			!
	  IF(IOS .NE. 0)ALLOCATE (H_INDX(ND,STAT=IOS)			!
	  IF(IOS .NE. 0)THEN
...
! These values are computed, and returned.
!
	  IF(IOS .NE. 0)ALLOCATE (JNU(ND,STAT=IOS)
	  IF(IOS .NE. 0)ALLOCATE (RSQHNU(ND,STAT=IOS)
	  IF(IOS .NE. 0)ALLOCATE (dlnJdlnR(ND,STAT=IOS)
!
	  IF(IOS .NE. 0)ALLOCATE (AV_SIGMA(ND),STAT=IOS)
...
!
	  ...
	  ESEC=EXP(ESEC)
!
	  TA(1:ND)=LOG(FEDD_SM(1:ND)
	  CALL MON_INTERP(FEDD,ND,IONE,R,ND,TA,ND_SM,R_SM,ND_SM)
	  FEDD=EXP(FEDD)
!
	  TA(1:ND)=LOG(H_ON_J_SM(1:ND)
	  CALL MON_INTERP(H_ON_J,ND,IONE,R,ND,TA,ND_SM,R_SM,ND_SM)
	  H_ON_J=EXP(H_ON_J)
!
	  TA(1:ND)=LOG(N_ON_J_SM(1:ND)
	  CALL MON_INTERP(N_ON_J,ND,IONE,R,ND,TA,ND_SM,R_SM,ND_SM)
	  N_ON_J=EXP(FEDD)
!
	  TA(1:ND)=LOG(GEDD_SM(1:ND)
	  CALL MON_INTERP(GEDD,ND,IONE,R,ND,TA,ND_SM,R_SM,ND_SM)
	  GEDD=EXP(GEDD)
!
!
	ELSE
	  ...
	  FEDD(1:ND)=FEDD_SM(1:ND)
	  GEDD(1:ND)=GEDD_SM(1:ND)

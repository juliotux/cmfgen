...
!
	SUBROUTINE STEQ_ADVEC_V2(ID,HN_S,DI_S,N_S,R,VEL,
	...
	IF(LINEAR)THEN
	  DO K=1,ND
	    ...
	    DO I=1,N_S			!Which S.E. equationa
	      ...
	      SE(ID)%STEQ(I,K)=SE(ID)%STEQ(I,K)-DERIV_CONST*(T1-T2)/(R(K)-R(KP1))
	    END DO
	    T1=R(K)*R(K)*VEL(K)*DI_S(K)
	    T2=R(KP1)*R(KP1)*VEL(KP1)*DI_S(KP1)
	    SE(ID)%STEQ(EQION,K)=SE(ID)%STEQ(EQION,K)-DERIV_CONST*(T1-T2)/(R(K)-R(KP1))
	  END DO
!
	  IF(COMPUTE_BA)THEN
	    DO K=1,ND

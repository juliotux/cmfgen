...
!
        SUBROUTINE GET_POPS_AT_PREV_TIME_STEP_V5(POPS,OLD_POPS,OLD_R,DO_ADVECT,DO_RAD_DECAYS,
...
! independent variable. We multiply POPS by VOL_ECP_FAC since, this time,
! OLD_POPS has not been corrected for expansion.
!
	LOG_V=LOG(V)
	LOG_TMP_V=LOG(TMP_V)
	DO I=1,NT
	  IF(OLD_LEV_POP_AVAIL(I))THEN
	    TMP_VEC=LOG(TMP_POPS(I,:))
	    CALL MON_INTERP(NEW_VEC,ND,IONE,LOG_V,ND,TMP_VEC,ND_OLD,LOG_TMP_V,ND_OLD)
	    OLD_POPS(I,:)=EXP(NEW_VEC)
	  ELSE
	    OLD_POPS(I,:)=POPS(I,:)*VOL_EXP_FAC(:)
	  END IF	
	END DO
!

...
!            SUPER levels).
!
	SUBROUTINE VAR_OP_V8(VCHI,VETA,VCHI_NOT_IMP,VETA_NOT_IMP,
...
!
	    VCHI_NOT_IMP(EQION,K)=VCHI_NOT_IMP(EQION,K)-T1*SUM_ION_NOT_IMP/DI_S(ION_LEV,K)
	    VCHI_NOT_IMP(NT-1,K)=VCHI_NOT_IMP(NT-1,K)-T1*SUM_ION_NOT_IMP/ED(K)
	    VCHI_NOT_IMP(NT,K)=VCHI_NOT_IMP(NT,K)+T1*SUM_T1_NOT_IMP-SUM_T2_NOT_IMP/T(K)
C
	    T1=T1*TETA1

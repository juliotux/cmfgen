...
!
	        ...
	        dRATE_dUP=T4*U_STAR_RATIO(K,SIM_INDX)*(LINE_EMIS_CON(SIM_INDX)*LINE_QW_SUM(SIM_INDX)+STIM_FAC*JBAR_SIM(K,SIM_INDX))
...
!
	      IF(INCLUDE_dSLdT)THEN
	        ...
	        J=DIAG_INDX
	        DO K=1,ND
	          MUL_FAC=LINE_QW_SIM(SIM_INDX)*CHIL_MAT(K,SIM_INDX)/ETAL_MAT(K,SIM_INDX)
...
!	      IF(NEW_LINE_BA)THEN
	        DO L=INDX_BA_METH,ND
	          ...
	          dRATE_dUP=T3*U_STAR_RATIO(L,SIM_INDX)*(LINE_EMIS_CON(SIM_INDX)*LINE_QW_SUM(SIM_INDX)+STIM_FAC*JBAR_SIM(L,SIM_INDX))
